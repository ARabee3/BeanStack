...
function renderOrder() {
    ...
    if (keys.length === 0) {
        ...
        totRow.classList.add('d-none'); return;
    }
    empty.classList.add('d-none');
    let total = 0;
    list.innerHTML = keys.map(id => {
        const item = order[id]; total += item.price * item.qty;
...
function confirmOrder() {
    ...
    if (!user) { showToast('<i class="bi bi-person-x me-1"></i>Select an employee first!', 'warning'); return; }
    if (Object.keys(order).length === 0) { showToast('<i class="bi bi-exclamation-triangle me-1"></i>Order is empty!', 'warning'); return; }
    if (!document.getElementById('orderRoom').value) { showToast('<i class="bi bi-door-closed me-1"></i>Select a room!', 'warning'); return; }
